...
public interface YarnPlyRepository extends JpaRepository<YarnPly, Long> {

    @Query("""
           select e from YarnPly e
           where e.id = :id and e.organizationId = :orgId and e.deleted = false
           """)
    Optional<YarnPly> findScoped(@Param("id") Long id, @Param("orgId") Long orgId);
    ...
    @Query("""
           ...
           """)
    Page<YarnPly> search(@Param("orgId") Long orgId, @Param("q") String q, Pageable pageable);

    /** Dropdown feed: active rows only. */
    @Query("""
           select e from YarnPly e
           where e.organizationId = :orgId and e.active = true and e.deleted = false
           order by e.plyNumber asc
           """)
    List<YarnPly> lookup(@Param("orgId") Long orgId);

    boolean existsByOrganizationIdAndCodeIgnoreCaseAndDeletedFalse(Long organizationId, String code);

    boolean existsByOrganizationIdAndPlyNumberAndDeletedFalse(Long organizationId, Integer plyNumber);
}
