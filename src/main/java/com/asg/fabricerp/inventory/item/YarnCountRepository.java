...
public interface YarnCountRepository extends JpaRepository<YarnCount, Long> {
    ...
    @Query("""
           select e from YarnCount e
           where e.id = :id and e.organizationId = :orgId and e.deleted = false
           """)
    Optional<YarnCount> findScoped(@Param("id") Long id, @Param("orgId") Long orgId);
    ...
    @Query("""
           ...
           """)
    Page<YarnCount> search(@Param("orgId") Long orgId, @Param("q") String q, Pageable pageable);
    ...
    @Query("""
           ...
           order by e.name asc
           """)
    List<YarnCount> lookup(@Param("orgId") Long orgId);
