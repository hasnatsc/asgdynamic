...
public interface ItemBrandRepository extends JpaRepository<ItemBrand, Long> {
    ...
    @Query("""
           select e from ItemBrand e
           where e.id = :id and e.organizationId = :orgId and e.deleted = false
           """)
    Optional<ItemBrand> findScoped(@Param("id") Long id, @Param("orgId") Long orgId);
    ...
    @Query("""
           ...
           """)
    Page<ItemBrand> search(@Param("orgId") Long orgId, @Param("q") String q, Pageable pageable);
    ...
    @Query("""
           ...
           order by e.name asc
           """)
    List<ItemBrand> lookup(@Param("orgId") Long orgId);
